// Filename: sort-the-roster.user.js
// ==UserScript==
...
(function () {
  'use strict';
      ...
      /*
    AU-canvancements baseline notes (v11)
    - Canvas People/Roster page is SPA-like and re-renders the roster table; we debounce DOM mutations
      to avoid re-init storms and “blank page” failures.
    - tablesorter is initialized once per table instance; on subsequent mutations we trigger updateAll.
    - Clickable header UX is added via injected CSS; sort arrows are appended inside a wrapper span
      (.au-sort-label) so Canvas block-level header markup doesn’t push the arrow to a new line.
    - Event handlers are namespaced and de-duped (off/on) to prevent stacking after re-renders.
    - Optional debug logging can be enabled via `debug = true`.
  */
  ...
  // --- Minimal stability guards (avoid loops / re-init storms) ---
  let parsersAdded = false;
  let isRunning = false;
  let rerunTimer = null;
  ...
  function waitForRoster(mutations, observer) {
    ...
    if (!roster) {
      if (typeof observer === 'undefined') {
        const sel = document.getElementById('content') || document.body;
  ...
  function watchForUpdates(mutations, observer) {
    if (typeof observer === 'undefined') {
      // Canvas sometimes re-renders users view; selector can be null briefly
      const sel =
        document.querySelector('#content div[data-view="users"]') ||
  ...
  function scheduleRosterSort() {
    // Debounce: collapse multiple rapid mutations into one sort init/update
    if (rerunTimer) {
...
function rosterSort() {
  // Prevent re-entry if Canvas is thrashing the DOM
  if (isRunning) return;
  isRunning = true;
  ...
  try {
    ...
    if (columns === false) {
      // No roster rows yet; we’ll be called again on next mutation
      return;
    }

    // If tablesorter is already attached to the current table instance, just update.
    // (Canvas re-renders sometimes; in that case the new table won't have the class.)
    if (jq('table.roster').hasClass('tablesorter')) {
      jq('table.roster.tablesorter').trigger('updateAll', [true]);
    ...
    // UX: make headers look clickable (Canvas can override header styles; use !important)
    addCSS({
      'table.roster thead th':
        'cursor: pointer !important; color: #0374B5 !important; white-space: nowrap;',
      'table.roster thead th:hover': 'color: #025C8E !important;',
      'table.roster thead th:empty':
        'cursor: default !important; text-decoration: none !important; color: inherit !important;',

      // Wrap header content so label + arrow stay on one line
      'table.roster thead th .au-sort-label':
        'display: inline-flex; align-items: center; gap: 0.35em; text-decoration: underline !important; color: inherit !important;',

      'table.roster thead th .au-sort-indicator':
        'font-size: 0.85em; display: inline-block; line-height: 1; vertical-align: middle; color: #0374B5;',
    });


    // Add parsers once per page load (tablesorter keeps them globally)
    if (!parsersAdded) {
    ...
    const $table = jq('table.roster');

    // Avoid stacking event handlers if rosterSort runs again
    $table.off('sortEnd.auRosterSort');
    ...
    // Add arrows after each sort
    $table.on('sortEnd.auRosterSort', function () {
      updateSortIndicators();
    });

    // One immediate update for good measure
    jq('table.roster.tablesorter').trigger('updateAll', [true]);

    // Ensure indicators are correct immediately
    updateSortIndicators();
  } finally {
    isRunning = false;
  }
...
function updateSortIndicators() {
  const ths = document.querySelectorAll('table.roster thead th');

  ths.forEach(th => {
    // Remove any previous indicator we added
    const existing = th.querySelector('.au-sort-indicator');
    if (existing) existing.remove();

    // Skip “empty” headers (e.g., kebab/options column)
    const headerText = (th.textContent || '').trim();
    if (!headerText) return;

    // Tablesorter can use either naming convention depending on build
    const isAsc =
      th.classList.contains('tablesorter-headerAsc') || th.classList.contains('headerSortUp');
    const isDesc =
      th.classList.contains('tablesorter-headerDesc') || th.classList.contains('headerSortDown');

    // Ensure we have a wrapper that keeps label + arrow on one line
    let label = th.querySelector('.au-sort-label');
    if (!label) {
      label = document.createElement('span');
      label.className = 'au-sort-label';

      // Move all existing child nodes into the label wrapper (except our indicator)
      while (th.firstChild) {
    ...
    // Only show an arrow if this column is currently sorted
    if (!isAsc && !isDesc) return;

    const arrow = document.createElement('span');
    arrow.className = 'au-sort-indicator';
  ...
  function addCSS(styles) {
    if (typeof styles !== 'undefined' && Object.keys(styles).length > 0) {
      ...
      for (const key of keys) {
        // Fix: close the CSS rule properly
